...
export function TopNav() {
  ...
  useEffect(() => {
    ...
    async function loadModels() {
      ...
      try {
        ...
        const info: BackendModelInfo = await getModelInfo();

        const dashboardModels = getDashboardModels(info);

        setModels(dashboardModels);

        // ensure current model is valid
        if (!dashboardModels.some((m) => m.id === model)) {
          setModel(dashboardModels[0]?.id ?? "logistic");
        }
  ...
  useEffect(() => {
    ...
    if (!terms.includes(term)) {
      ...
      setTerm(
        terms[Math.min(4, terms.length - 1)] ??
        terms[0] ??
        "Term 1"
      );
